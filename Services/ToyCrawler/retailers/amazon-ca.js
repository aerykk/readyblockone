...
define([
    ...
    function findProduct(set, cb) {
        ...
        var url = 'http://www.amazon.ca/s/ref=nb_sb_noss_1?url=search-alias%3Daps&field-keywords='+ keyword.replace(/ /gi, '%20');
        xray(url, 'body', {})
        (function(err, _, $) {
            if (err) { console.trace(err); cb && cb(err); return; }

            // TODO CHECK FOR ONE WITH RATINGS AS LEGIT
    ...
    function getProduct(url, setNumber, cb) {
        ...
        (function(err, product, $) {
            ...
            try {
                ...
                if (product.currentPrice)
                    product.currentPrice = parseFloat(product.currentPrice.replace(/,/gi, '').replace(/^\s+|\s+$/g, '').replace('CDN$ ', ''));

                if (product.originalPrice)
                    product.originalPrice = parseFloat(product.originalPrice.replace(/,/gi, '').replace(/^\s+|\s+$/g, '').replace('CDN$ ', ''));
                else
        ...
        (function(err, items, $) {
            ...
            $('.s-item-container').each(function() {
                items.push({
                    ...
                    title: $(this).find('a.a-link-normal').text()
                });
            });
